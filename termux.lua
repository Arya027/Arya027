...
espo/ espo.py
@Espoofficial
Espoofficial Rename topixsb.py to espo.py
 1 contributor
616 baris (601 slok)  46,3 KB
impor  os , waktu , sys
impor  httpx
impor  acak
dari  warna impor colr  
impor  json
coba :
    print ( f"Versi ini adalah { '9.1' } " )
    print ( 'Sudah Support VIP Banyak Email, dan Tools Bisa untuk Open JOKI' )
# waktu.tidur('3')
# jika httpx.get('https://raw.githubusercontent.com/atr19love/rilis/master/versi.txt').text != '9.1':
# os.unlink('topixsb.py')
# dengan open('topixsb.py', 'w') sebagai file1:
# file1.write(httpx.get('https://raw.githubusercontent.com/atr19love/rilis/master/topixsb.py').text)
# print(f"Memperbarui TopixSB Termux Tools {httpx.get('https://raw.githubusercontent.com/atr19love/rilis/master/versi.txt').teks} Versi")
...
# KELUAR()
                ...
                jika  data [ 'mode' ] ==  '1' :
                 ...
                 data elif [ 'mode' ] ==  '4' :
                    ...
                    lain :
                        data [ 'modus' ] =  '1'
                        data [ 'kodewarna' ] = [ '255' , '0' , '0' ]
            ''  +=  warna ( huruf , depan = ( data [ 'kodewarna' ][ '0' ], data [ 'kodewarna' ][ '1' ], data [ 'kodewarna' ][ '2' ]), belakang = ( '0' , '0' , '0' ))
            koda  = []
            untuk  t  dalam  rentang ( '3' ):
                if  len ( hex ( data [ 'kodewarna' ][ t ])[ '2' :]) ==  '1' :
                    hex ( data [ 'kodewarna' ][ t ])[ '2' :] +=  '0'
                koda . tambahkan ( hex ( data [ 'kodewarna' ][ t ])[ '2' :])
            data [ 'kodewarnaCPM' ] +=  f'[ { kodas [ 0 ] } { kodas [ 1 ] } { kodas [ 2 ] } ] { huruf } '
        print ( f"hasil \t :   { dispwarna ( data [ 'kodewarnaCPM' ]) } " )
        print ( f"kode \t :   { data [ 'kodewarnaCPM' ] } " )
        mengembalikan  data [ 'kodewarnaCPM' ]

    def  dispwarna ( clrnama ):
        clrVnama  =  clrnama . pisahkan ( '[' )
        untuk  clrx  di  clrVnama :
            jika  Salah  ==  Salah :
                clrcode1  =  f' { clrx [ 0 : 2 ] } '
                clrcode2  =  f' { clrx [ 2 : 4 ] } '
                clrcode3  =  f' { clrx [ 4 : 6 ] } '
                ...
                clrVnama [ '0' ] +=  warna ( clrx [ '7' : '8' ], depan = ( clrcode1 , clrcode2 , clrcode3 ), belakang = ( '0' , '0' , '0' ))
            jika  Salah :
        clrVnama [ '0' ] +=  clrVnama [ len ( clrVnama ) -  '1' ][ '8' : len ( clrVnama [ len ( clrVnama ) -  '1' ])]
        kembalikan  clrVnama [ '0' ]

    def  warna ( warna , teks ):
        untuk  saya  dalam  teks :
            ''  +=  wna  +  i
        mengembalikan  dispwarna ( '' )
    inp  =  input ( 'apakah anda setuju? \n ketik y jika setuju \n ketik n jika tidak setuju \n pilihan :' )
    jika  dip . lebih rendah () !=  'y' :
        keluar ()
    ...
    def GetPlayerRecords():
        ...
        heder = {'Host': 'us-central1-cp-multiplayer.cloudfunctions.net', 'authorization': f"Bearer {dat['']}", 'firebase-instance-': dat['firebase-instance-'], 'content-type': 'application/json; chatset=utf-8', 'accept-encoding': 'gzip', 'User-Agent': f'Dalvik/2.1.0 (Linux; U; Android 8.1.0; ASUS_X00TD MIUI/16.2017.2009.087-20{random.randint(111111, 999999)})'}
        data = {'data': 'ios5'}
    ...
    def SavePlayerRecords7():
        ...
        heder = {'Host': 'us-central1-cp-multiplayer.cloudfunctions.net', 'authorization': f"Bearer {dat['']}", 'firebase-instance-': dat['firebase-instance-id-token'], 'content-type': 'application/json; chatset=utf-8', 'accept-encoding': 'gzip', 'User-Agent': f'Dalvik/2.1.0 (Linux; U; Android 8.1.0; ASUS_X00TD MIUI/16.2017.2009.087-20{random.randint(111111, 999999)})'}
            ...
            if dat['login'] == False:
                ...
                while True:
                    ...
                    if pil == '' or pil.lower() == 'enter':
                        ...
                        print(f"{warna('[00ffff]', '>>>>>>>>>>>>>>>>>>> Save Data')}")
                        if SavePlayerRecords7() == True:
                            print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                        lain :
                            print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                     pil elif . lebih rendah () ==  'uhh' :
                        jelas ()
                        print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Aktifkan Klakson Klakson' ) } " )
                        dat [ 'pemain' ][ 'mengapung' ][ '27' ] =  '1'
                        dat [ 'pemain' ][ 'mengapung' ][ '28' ] =  '1'
                        dat [ 'pemain' ][ 'mengapung' ][ '29' ] =  '1'
                        dat [ 'pemain' ][ 'mengapung' ][ '30' ] =  '1'
                        dat [ 'pemain' ][ 'mengapung' ][ '31' ] =  '1'
                        print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Simpan Data' ) } " )
                        jika  SavePlayerRecords7 () ==  Benar :
                            print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                        lain :
                            print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                     pil elif . lebih rendah () ==  'cm' :
                        jelas ()
                        print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Ganti Uang' ) } " )
                        dat [ 'pemain' ][ 'uang' ] =  int ( input ( 'Uang Baru : ' ))
                        print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Simpan Data' ) } " )
                        jika  SavePlayerRecords7 () ==  Benar :
                            print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                        lain :
                            print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                     pil elif . lebih rendah () ==  'cc' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            jelas ()
                            print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Ubah Koin' ) } " )
                            dat [ 'pemain' ][ 'koin' ] =  input ( 'Koin Baru : ' )
                            print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Simpan Data' ) } " )
                            jika  SavePlayerRecords7 () ==  Benar :
                                print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                            lain :
                                print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                        lain :
                            print ( warna ( '[FF0000]' , 'Daftar Email Anda ke VIP Terlebih Dahulu' ))
                     pil elif . lebih rendah () ==  'ua' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            jelas ()
                            print ( f" { warna ( '[00ffff]' , '>>>>>>>>>>>>>>>>>>> Tingkatkan Pencapaian' ) } " )
                            contoh  = { 't_distance' : '10000' , 'time' : '30000' , 'speed_banner' : '1000' , 'gifts' : '100' , 'treasure' : '100' , 'cars' : '137' , 'race_win' : '1000' , 'levels' : '82' , 'drift' : '1000' , 'run' : '500' ,'polisi'  : '1000' , 'block_post' :'1000' , 'real_estate' :'12' , 'fuel' : '10000' , 'car_trade' : '100' , 'car_exchange' : '100' , 'burnt_tire' : '100' , 'car_fix' : '100' , 'car_wash' : '100 ' , 'offroad' : '1000' , 'passanger_distance' : '1000' , 'reaksi' : '2000' , 'drift_max' : '1000' , 'taksi': '1000' , 'delivery' : '1000' , 'cargo' : '1000' , 'push_ups' : '957' , 'slicer_cut' : '1' , 'car_collided' : '1' , 'new_type' : ' 0' }
                            isidata  = { 'RatingData' : {}}
                            untuk  x  dalam  contoh :
                                cetak ( f' { '1' } . { x } ' )
                                '1'  +=  '1'
                            ...
                            jika  SetUserRatingCall ( isidata ) ==  True :
                                print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                            lain :
                                print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                        lain :
                            print ( warna ( '[FF0000]' , 'Daftar Email Anda ke VIP Terlebih Dahulu' ))
                     pil elif . lebih rendah () ==  'irk' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            jelas ()
                            print ( warna ( '[00ffff]' , '=================== Istant King Rank' ))
                            contoh  = { 't_distance' : '10000' , 'time' : '30000' , 'speed_banner' : '1000' , 'gifts' : '100' , 'treasure' : '100' , 'cars' : '137' , 'race_win' : '1000' , 'levels' : '82' , 'drift' : '1000' , 'run' : '500' ,'polisi'  : '1000' , 'block_post' :'1000' , 'real_estate' :'12' , 'fuel' : '10000' , 'car_trade' : '100' , 'car_exchange' : '100' , 'burnt_tire' : '100' , 'car_fix' : '100' , 'car_wash' : '100 ' , 'offroad' : '1000' , 'passanger_distance' : '1000' , 'reaksi' : '2000' , : '1000' , 'pengiriman' : '1000' , 'kargo' : 'drift_max' : '1000' , 'taksi''1000' , 'push_ups' : '957' , 'slicer_cut' : '1' , 'car_collided' : '1' , 'new_type' : ' 0' }
                        ...
                        lain :
                            print ( warna ( '[FF0000]' , 'Daftar Email Anda ke VIP Terlebih Dahulu' ))
                     pil elif . lebih rendah () ==  'csc' :
                        coba :
                            ' \t Script ini membutuhkan data login akun anda \n     \t untuk dapat mengedit value pada akun anda \n     \t jika anda tidak setuju maka tinggalkan script ini \n     '  =  TestGetAllCars ()
                            untuk  idcar  di  dat [ 'datacar' ]:
                                print ( warna ( '[00FFFF]' , '-----------------------' ))
                                spesifikasi  = []
                                untuk  dataspek  dalam  range ( '1' , '5' ):
                                    if  dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ dataspek ] !=  '0' :
                                        spek . tambahkan ( dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ dataspek ])
                                    lain :
                                        spek . tambahkan ( 'Default' )
                            ...
                            print ( warna ( '[00FFFF]' , '-----------------------' ))
                            idpil  =  input ( 'Pilih Nomor Mobil : ' )
                            coba :
                                idcar  =  cariid ( int ( idpil ))
                                idpil  =  int ( idcar )
                                jika  idpil  di  dat [ 'datacar' ]:
                                    ...
                                    print ( warna ( '[C533FF]' , 'Spesifikasi Baru:' ))
                                    hp  =  masukan ( 'HP : ' )
                                    ihp  =  input ( 'Hp Dalam : ' )
                                    nm  =  masukan ( 'NM : ' )
                                    inm  =  input ( 'NM dalam: ' )
                            ...
                            kecuali :
                                print ( 'silakan ketik Nomor MOBIL' )
                        kecuali  Pengecualian  sebagai  e :
                            cetak ( f'Error : { e } ' )
                     pil elif . lebih rendah () ==  'mc' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            ' \t Script ini membutuhkan data login akun anda \n     \t untuk dapat mengedit value pada akun anda \n     \t jika anda tidak setuju maka tinggalkan script ini \n     '  =  TestGetAllCars ()
                            untuk  idcar  di  dat [ 'datacar' ]:
                                print ( warna ( '[00FFFF]' , '-----------------------' ))
                            ...
                            coba :
                                ...
                                jika  idpil  di  dat [ 'datacar' ]:
                                    print ( f" \t HP : { warna ( '[00FF00]' , str ( dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ 1 ])) } ( { warna ( '[ C3FF00]' , str ( dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ 2 ])) } )" )
                                    print ( f" \t NM : { warna ( '[00FF00]' , str ( dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ 3 ])) } ( { warna ( '[ C3FF00]' , str ( dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ 4 ])) } )" )
                                    dat [ 'datacar' ][ idpil ][ 'data' ][ 'vektor' ][ '6' ][ 'x' ] =  float ( '9898989898' )
                                    if  SaveCars ( dat [ 'datacar' ][ idpil ]) ==  True :
                                        print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                                    lain :
                                        print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                        ...
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            ...
                            coba :
                                ...
                                if idpil in dat['datacar']:
                                    print(f"\tHP : {warna('[00FF00]', str(dat['datacar'][idpil]['data']['floats'][1]))}({warna('[C3FF00]', str(dat['datacar'][idpil]['data']['floats'][2]))})")
                                    print(f"\tNM : {warna('[00FF00]', str(dat['datacar'][idpil]['data']['floats'][3]))}({warna('[C3FF00]', str(dat['datacar'][idpil]['data']['floats'][4]))})")
                                    dat [ 'datacar' ][ idpil ][ 'data' ][ 'vektor' ][ '6' ] = { 'x' : '2' , 'y' : '2' , 'z' : '2' }
                                    if  SaveCars ( dat [ 'datacar' ][ idpil ]) ==  True :
                                        print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                                    lain :
                                        print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
                     ...
                     pil elif . lebih rendah () ==  'csac' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            ...
                            untuk  idpil  di  dat [ 'datacar' ]:
                                ...
                                print ( f" \t HP : { warna ( '[00FF00]' , str ( spek [ 0 ])) } ( { warna ( '[C3FF00]' , str ( spek [ 1 ])) } )" )
                                print ( f" \t NM : { warna ( '[00FF00]' , str ( spek [ 2 ])) } ( { warna ( '[C3FF00]' , str ( spek [ 3 ])) } )" )
                                print ( warna ( '[C533FF]' , f' Spesifikasi Baru { idpil } :' ))
                                print ( f" \t HP { warna ( '[00FF00]' , hp ) } ( { warna ( '[C3FF00]' , ihp ) } )" )
                                print ( f" \t NM { warna ( '[00FF00]' , nm ) } ( { warna ( '[C3FF00]' , inm ) } )" )
                                dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ '1' ] =  hp
                                dat [ 'datacar' ][ idpil ][ 'data' ][ 'floats' ][ '2' ] =  ihp
                        ...
                        lain :
                            print ( warna ( '[FF0000]' , 'Daftar Email Anda ke VIP Terlebih Dahulu' ))
                     pil elif . lebih rendah () ==  'ccac' :
                        jika  dat [ 'hibah' ] ==  'Vip' :
                            pil  =  input ( '1. Chrome AKTIF \n 2. Chrome NONAKTIF \n pilihan : ' )
                            ' \t Script ini membutuhkan data login akun anda \n     \t untuk dapat mengedit value pada akun anda \n     \t jika anda tidak setuju maka tinggalkan script ini \n     '  =  TestGetAllCars ()
                            print ( warna ( '[00FFFF]' , '-----------------------' ))
                            untuk  idpil  di  dat [ 'datacar' ]:
                                coba :
                                    ...
                                    print ( f" \t NM : { warna ( '[00FF00]' , str ( spek [ 2 ])) } ( { warna ( '[C3FF00]' , str ( spek [ 3 ])) } )" )
                                    jika  pil  ==  '1' :
                                        dat [ 'datacar' ][ idpil ][ 'data' ][ 'vektor' ][ '12' ] = { 'x' : - '4' , 'y' : - '4' , 'z' : - ' 4' }
                                    elif  pil  ==  '2' :
                                        dat [ 'datacar' ][ idpil ][ 'data' ][ 'vektor' ][ '12' ] = { 'x' : '2' , 'y' : '2' , 'z' : '2' }
                                    if  SaveCars ( dat [ 'datacar' ][ idpil ]) ==  True :
                                        print ( f" { warna ( '[00ff00]' , 'Sukses' ) } " )
                                    lain :
                                        print ( f" { warna ( '[ff0000]' , 'Gagal' ) } " )
...
Blog
About
espo/espo.py at 02518674aa3966505ecf2b18372c17dddc1b3e42 · Espoofficial/espo